(function() {
	...
	d.formatDuration = function(a, b, c, e, g, f) {
		...
		"mm" == b && (a = d.roundTo(a, f.precision)); ("mm" == b || "hh" == b) && 10 > a && (a = "0" + a);
		c = a + "" + e[b] + "" + c;
	...
	d.AmChart = d.Class({
		...
		afterWrite: function(a) {
			...
			if (b = "object" != typeof a ? document.getElementById(a) : a) {
				...
				this.listenersAdded || (this.addListeners(), this.listenersAdded = !0); (this.mouseWheelScrollEnabled || this.mouseWheelZoomEnabled) && d.addWheelListeners();
				this.initChart()
			}
		},
		...
		brr: function() {
			if (!this.hideCredits) {
				...
				if (c != a || !0 === this.inIframe()) {
					...
					e = "数据来源：国家数据";
					"ammap" == this.product && (c = a + "/javascript-maps/", b = "Interactive JavaScript maps", e = "数据来源：国家数据");
	...
	d.makeHD = function(a, b, c) {
		for (var e = [], d = [], f = 1; f < a.length; f++) for (var h = Number(a[f - 1]), k = Number(b[f - 1]), l = Number(a[f]), m = Number(b[f]), n = Math.round(Math.sqrt(Math.pow(l - h, 2) + Math.pow(m - k, 2)) / 50) + 1, l = (l - h) / n, m = (m - k) / n, p = 0; p <= n; p++) {
	...
	d.handDrawnLine = function(a, b, c, e, g, f, h, k, l, m) {
		var n, p = a.set();
		for (n = 1; n < b.length; n++) for (var r = [b[n - 1], b[n]], t = [c[n - 1], c[n]], t = d.makeHD(r, t, a.handDrawScatter), r = t[0], t = t[1], q = 1; q < r.length; q++) p.push(d.line(a, [r[q - 1], r[q]], [t[q - 1], t[q]], e, g, f + Math.random() * a.handDrawThickness - a.handDrawThickness / 2, h, k, l, m, !0));
		return p
	};
	...
	d.AmLegend = d.Class({
		...
		drawLegend: function() {
			...
			this.divWidth = c; (b = this.container) ? (b.container.innerHTML = "", f.appendChild(b.container), b.width = c, b.height = g, b.setSize(c, g), b.addDefs(a)) : b = new d.AmDraw(f, c, g, a);
			this.container = b;
			this.lx = 0;
			this.ly = 8;
			g = this.markerSize;
		...
		createEntry: function(a) {
			if (!1 !== a.visibleInLegend && !a.hideFromLegend) {
				...
				if (e) {
					...
					m.push(t),
					a.bullet && (a.hidden || (f = a.bulletColorR), t = d.bullet(p, a.bullet, a.bulletSize, f, a.bulletAlpha, a.bulletBorderThickness, k, a.bulletBorderAlpha)) && (d.setCN(c, t, "graph-bullet"), t.translate(r + 1, r / 2), m.push(t)),
					y = 0,
					t = r,
					q = r / 3;
					else {
					...
					var B;
					isNaN(b.gradientRotation) || (B = 180 + b.gradientRotation); (m = b.createMarker(g, f, h, void 0, void 0, void 0, B, l)) && m.translate(r / 2, r / 2)
				}
				...
				var w;
				f && (f = d.fixBrakes(f), a.legendTextReal = f, w = b.labelWidth, w = isNaN(w) ? d.text(b.container, f, k, c.fontFamily, l, "start") : d.wrappedText(b.container, f, k, c.fontFamily, l, "start", !1, w, 0), d.setCN(c, w, "legend-label"), w.translate(b.lx + t, b.ly), p.push(w), b.labelDx = t, c = w.getBBox().width, b.maxLabelWidth < c && (b.maxLabelWidth = c));
				b.entries[b.index] = p;
		...
		createValue: function(a) {
			...
			if (!1 !== a.visibleInLegend && !a.hideFromLegend) {
				...
				if (b.valueText && 0 < b.valueWidth) {
					var h = b.color;
					b.useMarkerColorForValues && (h = a.color, a.legendKeyColor && (h = a.legendKeyColor())); ! 0 === a.hidden && (h = b.markerDisabledColor);
					var k = b.valueText,
			...
			if (a) for (c = 0; c < a.length; c++) {
				...
				if (e) f.value ? g.text(f.value) : g.text("");
					...
					if (!f.pattern && (this.useMarkerColorForValues && g.setAttr("fill", n), this.useMarkerColorForLabels)) {
						...
						g.setAttr("stroke", n); (g = f.legendLabel) && (f.hidden ? g.setAttr("fill", this.markerDisabledColor) : g.setAttr("fill", n))
					}
				}
			}
		},
	...
	d.AmMap = d.Class({
		...
		update: function() {
			if (d.hasSVG) {
				...
				for (var a = 0,
				b = this.updatableImages.length; a < b; a++) this.updatableImages[a].update()
			}
		},
		animateMap: function() {
			var a = this;
		...
		rescaleMapAndObjects: function() {
			...
			if (this.areasSettings.adjustOutlineThickness) {
				for (var b = this.svgAreas,
				d = 0; d < b.length; d++)(c = b[d]) && c.setAttr("stroke-width", this.areasSettings.outlineThickness / a / this.mapScale);
		...
		handleReleaseOutside: function() {
			if (d.isModern) {
				...
				if (!a.preventHover) {
					...
					a.fire(b)); (a.mouseIsOver && !a.mapWasDragged && !a.skipClick || a.wasTouched && 3 > Math.abs(a.mouseX - a.tmx) && 3 > Math.abs(a.mouseY - a.tmy)) && a.fire({
						type: "click",
						x: a.mouseX,
						y: a.mouseY,
						chart: a
		...
		showAsRolledOver: function(a) {
			...
			if (!a.showAsSelected && b && !a.isOver) {
				...
				if (void 0 != c) if ("MapImage" == a.objectType)(e = a.image) && e.setAttr("fill", c);
					...
					e.setAttr("stroke", c)
				} else b.setAttr("fill", c); (c = a.imageLabel) && !a.labelInactive && (e = a.labelRollOverColorReal, void 0 != e && c.setAttr("fill", e));
				...
				if ("MapArea" == a.objectType) {
					...
					isNaN(g) || b.setAttr("stroke-width", g / e); (c = c.rollOverPattern) && b.pattern(c, this.mapScale, this.path)
				}
	...
	d.ImagesProcessor = d.Class({
		...
		createImage: function(a, b) {
			...
			t = a.outlineThickness;
			isNaN(t) && (t = y); (y = a.labelPosition) || (y = v);
			v = a.labelColor;
			void 0 == v && (v = A);
			...
			G = a.labelColorReal; ! a.showAsSelected && c.selectedObject != a || void 0 == h || (D && D.setAttr("fill", h), G = a.selectedLabelColorReal);
			D = null;
		...
		updateSizeAndPosition: function(a) {
			...
			else if (!isNaN(l) && !isNaN(k)) if (g = b.coordinatesToXY(k, l), e = g.x, g = g.y, isNaN(p) || (e = p), isNaN(r) || (g = r), a.fixedSize) {
				p = a.positionScale;
		...
		createPredefinedImage: function(a, b, c, e, g, f) {
			...
			case "hexagon":
				g /= Math.sqrt(3),
				k = d.polygon(h, [.866 * g, 0 * g, -.866 * g, -.866 * g, 0 * g, .866 * g], [.5 * g, 1 * g, .5 * g, -.5 * g, -1 * g, -.5 * g], a, 1, c, b, 1)
			}
			return k
		},
		reset: function() {
	...
	d.LinesProcessor = d.Class({
		...
		createLine: function(a, b) {
			...
			if (a.remainVisible || c.selectedObject == a.parentObject) {
				...
				e = a.latitudes;
				q = n.length;
				if (0 < q) for (v = [], C = [], u = 0; u < q; u++) B = c.coordinatesToXY(n[u], e[u]),
				v.push(B.x),
				C.push(B.y);
				if (0 < v.length) {
					a.segments = v.length;
					...
					if (isNaN(q)) {
						for (q = 0; q < v.length - 1; q++) H = Math.sqrt(Math.pow(v[q + 1] - v[q], 2) + Math.pow(C[q + 1] - C[q], 2)),
						a.distances[q] = H;
					...
					if ("none" != A) {
						var I, K, N;
						if ("end" == A || "both" == A) u = v[v.length - 1],
						D = C[C.length - 1],
						...
						if ("start" == A || "both" == A) r = v[0],
						N = C[0],
						1 < v.length ? (u = v[1], K = C[1]) : (u = r, K = N),
	...
	d.normalizePath = function(a) {
		for (var b = "",
		c = d.parsePath(a.getAttribute("d")), e, g, f = Infinity, h = -Infinity, k = Infinity, l = -Infinity, m = 0; m < c.length; m++) {
			var n = c[m],
			p = n.letter,
			r = n.x,
			n = n.y;
	...
	d.eckert6 = function(a, b) {
		for (var c = Math.PI,
		d = (1 + c / 2) * Math.sin(b), g = 0, f = Infinity; 10 > g && 1E-5 < Math.abs(f); g++) b -= f = (b + Math.sin(b) - d) / (1 + Math.cos(b));
		d = Math.sqrt(2 + c);
